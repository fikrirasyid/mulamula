	...
	if( has_post_thumbnail() ){		
		...
		if( isset( $featured_image ) && $featured_image != '' ){
			...
			if( in_array( $mime_type, array( 'jpg', 'jpeg' ) ) ){
				$luminance = mulamula_get_avg_luminance( $featured_image[0] );				
			} else {
				$luminance = 200;
			}
		} else {
...
<header id="header" class="wrap-outer single dark">
	<div class="wrap">
		...
		<h2 id="site-desc" class="page-theme-description"><a href="<?php echo home_url(); ?>" title="<?php _e( '&larr; Back to post', 'mulamula' ); ?>"><?php _e( '&larr; Back to post', 'mulamula' ); ?></a></h2>			
	</div>
</header>
...
<header id="header" class="wrap-outer single <?php echo $color_scheme; ?>">
	<div class="wrap">
		<h1 id="site-name" class="page-theme"><a href="<?php echo esc_url( get_category_link( $categories[0]->cat_ID ) ); ?>" title="<?php printf( __( 'View all %1$s Posts', 'mulamula' ), $categories[0]->name ); ?>" rel="home"><?php echo $categories[0]->name; ?></a></h1>
