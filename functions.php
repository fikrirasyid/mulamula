...
function mulamula_setup(){
	...
	register_nav_menus( array(
		'top_nav' => __( 'Top Navigation', 'mulamula' )
	) );
...
function mulamula_widget_setup(){
    /**
    * Register widget area
    */
    register_sidebar( array(
        'name' => __( 'Footer Widgets', 'mulamula' ),
        'id' => 'footer-widgets',
        'description' => __( 'Widgets in this area will be shown on Footer Area', 'mulamula' ),
    ) );
}
add_action( 'widgets_init', 'mulamula_widget_setup' );
...
function mulamula_scripts(){
	wp_enqueue_style( 'mulamula_google_fonts', 'http://fonts.googleapis.com/css?family=Open+Sans:400,700|Montserrat:300,400,700' );
	wp_enqueue_style( 'mulamula_style', get_template_directory_uri() . '/css/screen.css' );

	wp_enqueue_script( 'mulamula_script', get_template_directory_uri() . '/js/script.js', array( 'jquery' ), '20131106', true );
 ...
 */
add_filter( 'post_thumbnail_html', 'mulamula_remove_width_attribute', 10 );
add_filter( 'image_send_to_editor', 'mulamula_remove_width_attribute', 10 );

function mulamula_remove_width_attribute( $html ) {
